...
import ManageEarningsTable from './components/ManageEarningsTable.tsx';


const ManageEarnings: FC = (): ReactElement => {
  const { orders, seller } = useOutletContext<SellerContextType>();
  console.log(orders);
  const completedOrders: IOrderDocument[] = filter(orders, (order: IOrderDocument) => lowerCase(order.status) === lowerCase('Completed'));
