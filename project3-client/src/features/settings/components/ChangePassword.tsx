...
import { PASSWORD_TYPE } from '../../../shared/utils/static-data.ts';
  ...
  const updatePassword = async (): Promise<void> => {
    try {
      if (!authUser.username) {
        setAlertMessage('User information not found');
        showErrorToast('User information not found');
        return;
      }

      if (passwordItem.newPassword !== passwordItem.retypeNewPassword) {
        setAlertMessage('New passwords do not match');
        showErrorToast('New passwords do not match');
        return;
      }

      await changePassword({ currentPassword: passwordItem.currentPassword, newPassword: passwordItem.newPassword, username: authUser.username }).unwrap();
  ...
  return (
    <div>
      ...
      <>
        ...
        <label htmlFor="retypeNewPassword" className="text-sm font-bold leading-tight tracking-normal text-gray-800">
        ...
        <div className="relative flex gap-4">
          <TextInput
            ...
            type={passwordItem.passwordType}
            value={passwordItem.retypeNewPassword}
            className="mb-5 mt-2 flex h-10 w-full items-center rounded border border-gray-300 pl-3 text-sm font-normal text-gray-600 focus:border focus:border-sky-500/50 focus:outline-none"
            placeholder="Confirm new password"
            onChange={(event: ChangeEvent) => {
              setPasswordItem({ ...passwordItem, retypeNewPassword: (event.target as HTMLInputElement).value });
            }}
          />
          <div className="absolute right-0 flex h-full cursor-pointer items-center pr-3 text-gray-600">
            {passwordItem.passwordType === PASSWORD_TYPE.PASSWORD ? (
        ...
        <div className="flex w-full items-center justify-center">
          <Button
            className={`text-md block w-full cursor-pointer rounded px-8 py-2 text-center font-bold text-white focus:outline-none ${
              ...
              !passwordItem.retypeNewPassword ||
              passwordItem.newPassword !== passwordItem.retypeNewPassword
                ? 'cursor-not-allowed bg-sky-200'
                : 'bg-sky-500 cursor-pointer'
            }`}
            label="Save Changes"
            onClick={updatePassword}
            disabled={!passwordItem.currentPassword ||
              !passwordItem.newPassword ||
              !passwordItem.retypeNewPassword ||
              passwordItem.newPassword !== passwordItem.retypeNewPassword}
          />
