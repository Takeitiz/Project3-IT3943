...
import { IOrderDocument, IOrderTableProps } from '../../order/interfaces/order.interface.ts';
            ...
            {orders.map((order: IOrderDocument) => (
              <tr key={uuidv4()}
                ...
                <td className="px-3 py-3 lg:flex lg:justify-center">
                  <img className="h-6 w-10 object-cover lg:h-8 lg:w-11" src={order.gigCoverImage}
                       alt="Gig cover image" />
                </td>
                <td className="p-3 text-left">
                  <div className="grid">
                    <Link to={`/orders/${order.orderId}/activities`}
