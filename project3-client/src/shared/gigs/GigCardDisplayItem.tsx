import { IGigCardItems, ISellerGig } from '../../features/gigs/interfaces/gig.interface.ts';
import { FC, ReactElement, useEffect, useRef } from 'react';
import { Link, NavigateFunction, useNavigate } from 'react-router-dom';
import { lowerCase, rating, replaceAmpersandAndDashWithSpace, replaceSpacesWithDash } from '../utils/utils.service.ts';
...
const GigCardDisplayItem: FC<IGigCardItems> = ({ gig, linkTarget, showEditIcon }): ReactElement => {
  ...
  const saveGigTitle = (gig: ISellerGig): void => {
    if (authUser?.username) {
      const category: string = (gig.categories);
